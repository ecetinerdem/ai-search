...
func (d *AstarSearch) Add(i *Node) {
	...
	i.EstimatedCostToGoal = int(euclideanDist(i.State, d.Game.Goal) + float64(i.CostToGoal))
	if i.State.Water {
...
func (d *AstarSearch) Solve() {
	fmt.Println("Starting to solve maze using astar first search...")
...
func (d *AstarSearch) Neighbours(node *Node) []*Node {
	row := node.State.Row
	col := node.State.Col
